...
def trip_duration_stats(df):
    ...
    total_duration = df['Trip Duration'].sum() / (60 * 60)
    print("total travel time in hours is: ", total_duration)

    # TO DO: display mean travel time
    mean_duration = df['Trip Duration'].mean() / (60 * 60)
...
def display_raw_data(df):
    ...
    i = 0
    raw = input(
        "<Do you want to see the raw data? Yes or No?>").lower()  # TO DO: convert the user input to lower case using lower() function
    ...
    while True:
        if raw == 'no':
            ...
            raw = input(
                "<Do you want to see more? Yes or No>").lower()  # TO DO: convert the user input to lower case using lower() function
            i += 5
        else:
